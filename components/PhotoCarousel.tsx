...
export default function PhotoCarousel({ restaurantSlug, onUploadClick }: PhotoCarouselProps) {
  ...
  // Pause all videos when switching photos
  useEffect(() => {
            ...
            <div className="relative w-full h-full rounded-xl overflow-hidden shadow-2xl">
              {prevPhoto?.is_video ? (
                <div className="relative w-full h-full">
                  <video
                    ref={(el) => {
                      if (el && prevPhoto && !videoThumbnails[prevPhoto.id]) {
                        videoRefs.current[prevPhoto.id] = el
                              ...
                              const onSeeked = () => {
                                try {
                                  ...
                                  if (ctx) {
                                    ...
                                    setVideoThumbnails(prev => ({ ...prev, [prevPhoto.id]: thumbnail }))
                                  }
                                } catch (err) {
                        ...
                        if (el.readyState >= 2) {
                          ...
                          el.addEventListener('loadeddata', generateThumbnail, { once: true })
                        }
                      } else if (el && prevPhoto) {
                        videoRefs.current[prevPhoto.id] = el
                      }
                    }}
                    src={prevImageUrl}
                    className="w-full h-full object-cover"
                    poster={videoThumbnails[prevPhoto.id] || prevImageUrl}
                    muted
                    playsInline
                    preload="metadata"
                  />
                  {!playingVideos.has(prevPhoto.id) && (
                    <div className="absolute inset-0 flex items-center justify-center bg-black/20 pointer-events-none">
                      <div className="bg-white/90 rounded-full p-2 sm:p-3 shadow-lg">
                        <svg className="w-4 h-4 sm:w-6 sm:h-6 text-gray-900 ml-0.5" fill="currentColor" viewBox="0 0 24 24">
            ...
            >
              {currentPhoto.is_video ? (
                <div className="relative w-full h-full">
                  <video
                    ref={(el) => {
                      if (el && !videoThumbnails[currentPhoto.id]) {
                  ...
                  {!playingVideos.has(currentPhoto.id) && (
                    ...
                    >
                      <div className="bg-white rounded-full p-4 sm:p-6 shadow-2xl hover:bg-gray-50 transition-all hover:scale-110 group-hover:scale-110 touch-manipulation">
                        <svg 
            ...
            <div className="relative w-full h-full rounded-xl overflow-hidden shadow-2xl">
              {nextPhoto?.is_video ? (
                <div className="relative w-full h-full">
                  <video
                    ref={(el) => {
                      if (el && nextPhoto && !videoThumbnails[nextPhoto.id]) {
                        videoRefs.current[nextPhoto.id] = el
                        // Generate thumbnail from first frame
                        const generateThumbnail = () => {
                          try {
                            if (el.videoWidth > 0 && el.videoHeight > 0) {
                              ...
                              const onSeeked = () => {
                                try {
                                  ...
                                  if (ctx) {
                                    ctx.drawImage(el, 0, 0, canvas.width, canvas.height)
                                    const thumbnail = canvas.toDataURL('image/jpeg', 0.8)
                                    setVideoThumbnails(prev => ({ ...prev, [nextPhoto.id]: thumbnail }))
                                  }
                                } catch (err) {
                                  console.error('Error generating thumbnail:', err)
                                }
                      ...
                      } else if (el && nextPhoto) {
                        videoRefs.current[nextPhoto.id] = el
                      }
                    }}
                    src={nextImageUrl}
                    className="w-full h-full object-cover"
                    poster={videoThumbnails[nextPhoto.id] || nextImageUrl}
                    muted
                    playsInline
                    preload="metadata"
                  />
                  {!playingVideos.has(nextPhoto.id) && (
                    <div className="absolute inset-0 flex items-center justify-center bg-black/20 pointer-events-none">
                      <div className="bg-white/90 rounded-full p-2 sm:p-3 shadow-lg">
                        <svg className="w-4 h-4 sm:w-6 sm:h-6 text-gray-900 ml-0.5" fill="currentColor" viewBox="0 0 24 24">
                          <path d="M8 5v14l11-7z"/>
